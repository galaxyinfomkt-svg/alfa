...
export default function ServiceCard({ slug, name, description, icon, cta, citySlug }: ServiceCardProps) {
  ...
  return (
    <Link href={href} className="group block">
      <div className="card-hover bg-white rounded-2xl p-8 shadow-md border border-gray-100 h-full">
        <div className="w-16 h-16 bg-alfa-blue/10 rounded-xl flex items-center justify-center text-alfa-blue mb-5 group-hover:bg-alfa-gold group-hover:text-white transition-all duration-300">
          {iconMap[icon] || iconMap.painting}
        </div>
        <h3 className="text-xl font-bold text-alfa-blue mb-3 group-hover:text-alfa-gold transition-colors">
          {name}
        </h3>
